...
$(function() {
    ...
    //$('.reset').live('click',function(){
        //A função live foi desativada na versão 1.9 dp Jquery, e substituída pela função on
        ...
        $(this).find('.required').each(function(i,elm){	
            ...
            if ( $(this).hasClass('data') ){    
                ...
                if(sdata.length!=10)
                {
                    $(this).removeClass('valid').addClass('invalid');
                    $(this).select();
                    $(".errorData").html(dataMsg).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
                    valid = false;
                    //return false;   
                }
                var data        = sdata;
                ...
                if(valid){
                    ...
                    var hoje = new Date();                      
                    var diferencaAnos = hoje.getFullYear() - parseInt(ano);
                    if ( new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate()) < 
                         new Date(hoje.getFullYear(), parseInt(mes), parseInt(dia)) )
                        diferencaAnos--;
                    
                    if(diferencaAnos < 16){
                        $(this).removeClass('valid').addClass('invalid');
                        $(this).select();
                        dataMsg += " Idade deve ser maior que 16 anos!";
                        $(".errorData").html(dataMsg).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
                        valid = false;
                    }
                }
            ...
            if ( $(this).hasClass('cpf') ){                
                ...
                if ((cpf.charAt(9) != a[9]) || (cpf.charAt(10) != a[10]) || cpf.match(expReg))
                {
                    ...
                    $('.errorCPF').html(cpfMsg).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
                    valid = false;
                    //return false;
                }
                else{
            ...
            if($(this).hasClass('cnpj'))
            {
                ...
                if ((cnpj.charAt(12) != a[12]) || (cnpj.charAt(13) != a[13])){
                    ...
                    $("#errorCPF").html(cnpjMsg).fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
                    valid = false;
                    //return false;
                }
                else
